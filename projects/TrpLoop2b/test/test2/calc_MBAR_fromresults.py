...
[int(step), float(self.E), int(accept), int(self.state), int(self.sigma_noe_index), int(self.sigma_J_index), int(self.gamma_index)]
"""
...
for n in range(nsnaps):
  ...
  for k in range(K):
    for l in range(K):
      ...
      if (1):
          state, sigma_noe_index, sigma_J_index, gamma_index = traj[k]['trajectory'][n][3:]
          print 'state, sigma_noe_index, sigma_J_index, gamma_index', state, sigma_noe_index, sigma_J_index, gamma_index
          states_kn[k,n] = state
          sigma_noe = traj[k]['allowed_sigma_noe'][sigma_noe_index]
          sigma_J = traj[k]['allowed_sigma_J'][sigma_J_index]
          u_kln[k,l,n] = sampler[l].neglogP(0, state, sigma_noe, sigma_J, gamma_index)
