...
#sys.path.append('../../src')
sys.path.append('./')
from RestraintFile_pf import *

# read in atom pair indices
Ind = np.loadtxt('atom_ind.txt')
...
2	0.2795903
"""
restraint_data = np.loadtxt('exp_data.txt')
...
pdbfile = 'new_traj/pdb/state0.pdb' 
...
for i in range(Ind.shape[0]):
    ...
    protectionfactor        = restraint_data[i,1]
    r.add_line_pf(restraint_index, a1, topology, protectionfactor)
