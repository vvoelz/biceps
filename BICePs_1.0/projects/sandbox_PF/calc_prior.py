    ...
    for o in range(len(allowed_xcs)):
        for p in range(len(allowed_xhs)):
                for q in range(len(allowed_bs)):
                        infile_Nc='input/Nc/Nc_x%0.1f_b%d_state000.npy'%(allowed_xcs[o], allowed_bs[q])
                        infile_Nh='input/Nh/Nh_x%0.1f_b%d_state000.npy'%(allowed_xhs[p], allowed_bs[q])
...
protectionfactor_distributions = np.zeros((107, len(allowed_beta_c), len(allowed_beta_h), len(allowed_beta_0), len(allowed_xcs), len(allowed_xhs), len(allowed_bs)))
summ=dict()
#betas=[]
ques=[]
#sys.exit()
...
#							print 'model_protectionfactor[',m,j,k,x,y,z,']', model_protectionfactor[m,j,k,x,y,z]
							summ[i].append(model_protectionfactor[m,j,k,x,y,z])
#	print i, sum(summ[i])
	betas=(sum(summ[i])/2.0)
	print i, betas
#							sys.exit()
...
#print sse	
	for n in summ[i]:
		ques.append(np.log(betas) + (n/betas))
print sum(ques)
