...
#				 dbs=1.0, bs_min=3.0, bs_max=5.0,
				 use_reference_prior_noe=True,
                                 ...
                                 use_gaussian_reference_prior_PF=True):
...
#        print 'len(self.allowed_sigma_PF) =', len(self.allowed_sigma_PF)
        ...
        print 'Loading the 6-dimensional array of nuisance priors P(beta_c, beta_h, beta_0, xcs, xhs, bs) ...'
       # self.nuisance_prior = np.load('nuisance_priors_6dim.npy')  # VAV
	# self.nuisance_prior = np.load('sampled_states_array.npy')	# added by Yunhui 05/2017
#	self.nuisance_prior = nuisance_prior
#	self.nuisance_prior=np.load('neglog_prior.npy')
	self.nuisance_prior=np.load('b15.npy')	
...
#        print 'len(self.allowed_bs) =', len(self.allowed_bs)
        ...
        self.traj = PosteriorSamplingTrajectory(self.ensembles[0], self.allowed_sigma_noe, self.allowed_sigma_J,
				self.allowed_sigma_cs_H, self.allowed_sigma_cs_Ha, self.allowed_sigma_cs_N, self.allowed_sigma_cs_Ca,
				self.allowed_sigma_PF, self.allowed_gamma,
				self.allowed_beta_c, self.allowed_beta_h, self.allowed_beta_0,
				self.allowed_xcs, self.allowed_xhs, self.allowed_bs)		#GYH 03/2017
...
#            self.build_reference_prior_PF()
	    self.build_exponential_reference_prior_PF()	#Vince forgot to change this! 06/2017
...
#                ref_sigma_Ca[j] = global_ref_sigma_Ca
    ...
    def build_exponential_reference_prior_PF(self):                          #GYH
        ...
        *** VAV: NOTE that this reference potential probably should NOT be used for protection factors, PF! ***"""	# VAV
        
        for k in range(self.nensembles):
        
            print 'Computing exponential reference priors (PF) for ensemble', k, 'of', self.nensembles, '...'
            ensemble = self.ensembles[k]

            s0 = ensemble[0] # the first structure in the ensemble
            nprotectionfactor = len(s0.protectionfactor_restraints)
            ...
            for j in range(nprotectionfactor):
                running_total = np.zeros( s0.protectionfactor_restraints[j].model_protectionfactor.shape )
                for s in ensemble:
                    running_total += s.protectionfactor_restraints[j].model_protectionfactor   # a 6-dim array
                beta_PF_j = running_total/(nprotectionfactor+1.0)  
                for s in ensemble:
                    s.beta_PF.append( beta_PF_j ) 
        ...
        *** VAV: NOTE that this reference potential probably should NOT be used for protection factors, PF! ***"""      # VAV
        ...
        for k in range(self.nensembles):
            ...
            for j in range(nprotectionfactor):
                ...
                mean_PF_j  = np.zeros( s0.protectionfactor_restraints[j].model_protectionfactor.shape )
                sigma_PF_j = np.zeros( s0.protectionfactor_restraints[j].model_protectionfactor.shape )
                for s in ensemble:
                    mean_PF_j += s.protectionfactor_restraints[j].model_protectionfactor   # a 6-dim array
                mean_PF_j = mean_PF_j/(nprotectionfactor+1.0)
                for s in ensemble:
                    sigma_PF_j += (s.protectionfactor_restraints[j].model_protectionfactor - mean_PF_j)**2.0   # a 6-dim array
                sigma_PF_j = np.sqrt(sigma_PF_j)/(nprotectionfactor+1.0)
    
                for s in ensemble:
                    s.ref_mean_PF.append( mean_PF_j )
                    s.ref_sigma_PF.append( sigma_PF_j )

            # With the ref_mean_PF_j  and ref_sigma_PF_j values computed (and stored in each structure), now we can calculate the neglog reference potentials
            for s in ensemble:
...
#		print 's.sse_protectionfactor',[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index], s.sse_protectionfactor
#                print "result", result
	        result += (s.Ndof_protectionfactor)/2.0*self.ln2pi
#		print [new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]
#		result -= np.log(self.nuisance_prior[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]) # GYH: neglogP so -= 04/2017
                result += (self.nuisance_prior[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]) # GYH: test 06/2017
        # reference priors
        if self.use_reference_prior_noe:
        ...
        if self.use_reference_prior_PF:
            result -= s.sum_neglog_reference_priors_PF[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]		#GYH: 06/2017
        ...
        if self.use_gaussian_reference_prior_PF:
            result -= s.sum_gaussian_neglog_reference_priors_PF[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]          #GYH:06/2017
	    ...
	    print 's.sum_neglog_reference_priors_noe', s.sum_neglog_reference_priors_noe, 's.sum_neglog_reference_priors_H', s.sum_neglog_reference_priors_H, 's.sum_neglog_reference_priors_Ha',s.sum_neglog_reference_priors_Ha, 's.sum_neglog_reference_priors_N', s.sum_neglog_reference_priors_N, 's.sum_neglog_reference_priors_Ca', s.sum_neglog_reference_priors_Ca, 's.sum_neglog_reference_priors_PF[', new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index, ']', s.sum_neglog_reference_priors_PF[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]	#GYH	
#	    print 's.sum_gaussian_neglog_reference_priors_noe', s.sum_gaussian_neglog_reference_priors_noe, 's.sum_gaussian_neglog_reference_priors_H', s.sum_gaussian_neglog_reference_priors_H, 's.sum_gaussian_neglog_reference_priors_N', s.sum_gaussian_neglog_reference_priors_N, 's.sum_gaussian_neglog_reference_priors_Ca', s.sum_gaussian_neglog_reference_priors_Ca, 's.sum_gaussian_neglog_reference_priors_PF[',new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index, ']', s.sum_gaussian_neglog_reference_priors_PF[new_beta_c_index, new_beta_h_index, new_beta_0_index, new_xcs_index, new_xhs_index, new_bs_index]       #GYH: 06/2017 
