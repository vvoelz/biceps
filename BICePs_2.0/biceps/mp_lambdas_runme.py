...
energies = np.loadtxt('cineromycinB_QMenergies.dat')
data = biceps.sort_data('noe_J')
...
A = biceps.Analysis(states=100, resultdir=outdir,
    BSdir=outdir+'/BS.dat', popdir=outdir+'/populations.dat',
    picfile=outdir+'/BICePs.pdf')
A.plot()
######## Convergence #######
#C = biceps.Convergence(trajfile='traj_lambda1.00.npz',
#    maxtau=10000, nblock=5, nfold=10, nround=1000)
#C.get_autocorrelation_curves(method="block-avg", nblocks=5,
#    plot_traces=True)
