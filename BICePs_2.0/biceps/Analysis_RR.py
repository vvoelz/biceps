...
#NOTE:
"""
1) Made some style convention edits
2)
"""
...
class Analysis(object):
    ...
    def __init__(self, states=0, data=None, resultdir=None, BSdir='BS.dat',
            popdir='populations.dat', picfile='BICePs.pdf'):

    # NOTE: Made this look nicer
	self.states = states
	self.data = data
	...
	self.scheme = None
	self.traj,self.sampler = [],[]
	self.lam,self.f_df,self.P_dp = None,None,None
    ...
    ## TODO:
    # Do not need all these if statements. Here is a solution:
    # NOTE: This can be used for any instance of loading data or find which
    # restraint belongs to which class. Maybe with this we can get rid of the
    # function sort_data inside toolbox.py ?

    """
    # Create a dictionary inside Restraint.py  - inside the restraint class
    _restraints = {
        'cs_H' : 'sigma_cs_H',
        'cs_H' : 'sigma_cs_H',
        'cs_Ha' : 'sigma_cs_Ha',
    ...
    # This is a one liner for all the if statements below
    d_l = [ Restraint._restraints[str(file.split(".")[-1])] for file in input_data[0] ]
    ...
    def load_data(self, debug = True):
        # NOTE: I don't know why there are still indentation issues... I'll leave this be
    ...
    # NOTE: ['step', 'E', 'accept', 'state', [nuisance parameters]]
...
#		print
	...
	t0 = self.traj[0]
        ...
        if 'gamma' in self.scheme:
            for k in range(len(self.scheme)-1): # NOTE: Why is this len() - 1?
        	plt.subplot(r,c,k+2)
...
#            plt.xlim(0,max(t0['allowed_gamma']))
            ...
            for k in range(len(self.scheme)): # NOTE: Why is this NOT len() - 1?
                plt.subplot(r,c,k+2)
                plt.step(t0['allowed_sigma'][k], t0['sampled_sigma'][k], 'b-')
                plt.hold(True)
                xmax0 = [l for l,e in enumerate(t0['sampled_sigma'][k]) if e != 0.][-1]
