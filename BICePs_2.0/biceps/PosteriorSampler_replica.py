...
class PosteriorSampler(object):
    ...
    def __init__(self, ensemble, freq_write_traj=100.,
            freq_print=100., freq_save_traj=100., nreplica = 4, pf_prior = None):
        ...
        self.state = np.random(self.nstates, self.nreplica, replacement = True) # it's okay to have identical states???
...
#        np.save('compiled_nuisance_parameters.npy',self.nuisance_para)
    ...
    def compute_sse(self,states,debug=False):
        ...
        all_sse = []
        for rest_index in range(len(self.ensemble[0])):
            r = [[] for i in range(len(self.ensemble[0][rest_index].n))]  # observable
            exp_r = []
            sse = 0.0
            for j in range(len(self.ensemble[0][rest_index].n)):
                exp_r.append(self.ensemble[0][rest_index][j].exp)
            for state in new_state:
                s = self.ensemble[int(state)]
                for ind in range(len(s[rest_index].n)):
                    r[ind].append(s[rest_index][ind].model)
            avg_r = np.mean(r,axis=1)
            if 'allowed_gamma' in s[rest_index]._nuisance_parameters:
                if s[rest_index].use_log_normal_noe:
                    for i in range(len(s[rest_index].n)):
                        err = np.log(avg_r[i]/(parameters[rest_index][1]*exp_r[i]))
                        sse += s[rest_index][i].weight * err ** 2.0
                else:
                    for i in range(len(s[rest_index].n)):
                        err = parameters[rest_index][1] * exp_r[i] - avg_r[i]
                        sse += s[rest_index][i].weight * err ** 2.0
            ### This shall not work yet!!! #####
    ...
    def neglogP(self, new_state, parameters, parameter_indices, verbose=False):
        ...
        sse = self.compute_sse(new_state)
        ...
        for rest_index in range(len(s)):
            ...
            if 'allowed_gamma' in s[rest_index]._nuisance_parameters:
                result += sse[rest_index] / (2.0*parameters[rest_index][0]**2.0)
            ### haven't done anything on this part yet!!! Don't use it for now!!! ######
            elif 'allowed_beta_c' in s[rest_index]._nuisance_parameters:
            ...
            ##############################################################
                ...
                result += sse[rest_index] / (2.0*float(parameters[rest_index][0])**2.0)
...
#            print('new_E',new_E,'self.E',self.E)
		...
		if actual_sample_ind == len(temp_parameters):
            ...
            for state in self.new_state:
                self.traj.state_counts[int(state)] += 1
                self.traj.state_trace.append(int(state))
