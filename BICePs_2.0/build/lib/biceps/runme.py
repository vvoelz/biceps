...
for j in lambda_values:
    ...
    for i in range(energies.shape[0]):   # number of states
        ...
        for k in range(len(data[0])):   # number of experimental observables
            ...
            if verbose:
                print File
            R=init_res('cineromycinB_pdbs/0.fixed.pdb',lam,energies[i],File,ref[k],uncern=uncern[k],gamma=gamma)
            ensemble[-1].append(R)
...
#    print ensemble
    ...
    sampler.traj.process_results(os.path.join(outdir,'RR_traj_lambda%2.2f.npz'%lam))  # compute averages, etc.
    print '...Done.'
