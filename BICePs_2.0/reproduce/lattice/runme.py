...
path='micro_distances/distance/*txt'
states=15037
indices='distance_indices.dat'
exp_data='exp_distances.dat'
top='Gens/micro/0.pdb'
data_dir=path
#dataFiles = 'test_cs_H'
dataFiles = 'noe_dis'
out_dir=dataFiles

#p=Preparation('noe',states=states,indices=indices,exp_data=exp_data,top=top,data_dir=data_dir)
...
#sys.exit(1)
energies_filename =  'energy/micro/energy_-0.5.txt'
    ...
    for i in range(energies.shape[0]):
        ...
        for k in range(len(data[0])):
            ...
            if File.endswith('cs_H'):
                ...
                R = Restraint_noe('Gens/micro/0.pdb',ref='exp')   # good ref
                R.prep_observable(lam=lam, free_energy=energies[i],
                        filename=File)
