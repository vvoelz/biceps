...
for subset in range(fold):
    ...
    T1 = a['trajectory'][:half]
    T2 = a['trajectory'][half:dx*(subset+1)]
    T_total = a['trajectory'][:dx*(subset+1)]
...
for i in range(n_rest):
    ...
    plt.plot(x,all_JSD[i],'o-',color=colors[i], label=rest_type[i])
    plt.xlabel('dataset (%)')
    plt.ylabel('JSD')
