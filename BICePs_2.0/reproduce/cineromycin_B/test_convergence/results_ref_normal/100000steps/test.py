...
#        sys.exit()
    ...
    np.save('all_hist_m.npy',all_hist_m)
#    sys.exit()
    ...
    if 'gamma' in rest_type:
        for i in range(n_rest):
            if i == n_rest - 1:
                plt.subplot(n_rest,1,i+1)
                for k in range(len(all_hist_m[i])):
                    plt.step(a['allowed_gamma'],all_hist_m[i][k],'-')
                plt.xlabel('$\gamma$')
                plt.ylabel('$P(\gamma)$')
                plt.yticks([])
            else:
                plt.subplot(n_rest,1,i+1)
                for k in range(len(all_hist_m[i])):
                    plt.step(a['allowed_sigma'][i],all_hist_m[i][k],'-')
                if rest_type[i].find('cs') == -1:   # means it is not cs_xxx
