    ...
    for i in range(energies.shape[0]):
        ...
        for k in range(len(data[0])):
            ...
            if File.endswith('cs_H'):
                ...
                R = Restraint_J('cineromycinB_pdbs/%d.fixed.pdb'%i,ref='uniform', dlogsigma=np.log(1.01), sigma_min=0.05, sigma_max=20.0)  # good ref
                R.prep_observable(lam=lam, free_energy=energies[i],
                        filename=File)

            elif File.endswith('noe'):
                R = Restraint_noe('cineromycinB_pdbs/%d.fixed.pdb'%i,ref='exp',dlogsigma=np.log(1.01), sigma_min=0.05, sigma_max=5.0)   # good ref
                R.prep_observable(lam=lam, free_energy=energies[i],
