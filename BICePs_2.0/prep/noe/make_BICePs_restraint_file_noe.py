...
Ind = np.loadtxt('atom_pair_indices.dat')
...
2	0.2795903
"""
restraint_data = np.loadtxt('noe_distances.txt')
...
for j in range(15037):
	model_data = np.loadtxt('micro_distances/dis_state%d.txt'%j)
	r = RestraintFile_noe()
	...
	for i in range(Ind.shape[0]):
    		...
    		exp_distance        = restraint_data[i,1]
		model_distance	    = model_data[i]
    		r.add_line_noe(restraint_index, a1, a2, topology, exp_distance, model_distance)

#print r

	r.write('test_noe/state%d.noe'%j)
