...
#        r_noe.allowed_sigma_noe = r_noe.allowed_sigma_noe
#        r_J.allowed_sigma_J = r_J.allowed_sigma_J
#        r_cs_H.allowed_sigma_cs_H = r_cs_H.allowed_sigma_cs_H
#        r_cs_Ha.allowed_sigma_cs_Ha = r_cs_Ha.allowed_sigma_cs_Ha
#        r_cs_N.allowed_sigma_cs_N = r_cs_N.allowed_sigma_cs_N
#        r_cs_Ca.allowed_sigma_cs_Ca = r_cs_Ca.allowed_sigma_cs_Ca
#        r_noe.allowed_gamma = r_noe.allowed_gamma


        # keep track of what we sampled in a trajectory
        self.traj = PosteriorSamplingTrajectory(self.ensembles[0],
                r_noe.allowed_sigma_noe,
                r_J.allowed_sigma_J,
                r_cs_H.allowed_sigma_cs_H,
                r_cs_Ha.allowed_sigma_cs_Ha,
                r_cs_N.allowed_sigma_cs_N,
                r_cs_Ca.allowed_sigma_cs_Ca,
...
#	s.sse_protectionfactor = None
	if s.sse_protectionfactor is not None:			# trying to fix a future warning:"comparison to `None` will result in an elementwise object comparison in the future."
        ...
        if self.use_reference_potential_noe:
            result -= s.sum_neglog_reference_potentials_noe		#GYH
        if self.use_reference_potential_H:
            result -= s.sum_neglog_reference_potentials_H		#GYH
        if self.use_reference_potential_Ha:
            result -= s.sum_neglog_reference_potentials_Ha		#GYH
        if self.use_reference_potential_N:
            result -= s.sum_neglog_reference_potentials_N		#GYH
        if self.use_reference_potential_Ca:
            result -= s.sum_neglog_reference_potentials_Ca		#GYH
        if self.use_reference_potential_PF:
            result -= s.sum_neglog_reference_potentials_PF		#GYH

        if self.use_gaussian_reference_potential_noe:
            result -= s.sum_gaussian_neglog_reference_potentials_noe         #GYH
        if self.use_gaussian_reference_potential_H:
            result -= s.sum_gaussian_neglog_reference_potentials_H           #GYH
        if self.use_gaussian_reference_potential_Ha:
            result -= s.sum_gaussian_neglog_reference_potentials_Ha          #GYH
        if self.use_gaussian_reference_potential_N:
            result -= s.sum_gaussian_neglog_reference_potentials_N           #GYH
        if self.use_gaussian_reference_potential_Ca:
            result -= s.sum_gaussian_neglog_reference_potentials_Ca          #GYH
        if self.use_gaussian_reference_potential_PF:
            result -= s.sum_gaussian_neglog_reference_potentials_PF          #GYH
    ...
    def sample(self, nsteps):
	    ...
	    new_alpha_index = self.alpha_index	#GYH
            ...
            if np.random.random() < 0.16:
                ...
                new_sigma_noe_index = new_sigma_noe_index%(len(r_noe.allowed_sigma_noe)) # don't go out of bounds
                new_sigma_noe = r_noe.allowed_sigma_noe[new_sigma_noe_index]

            elif np.random.random() < 0.32:
                # take a step in array of allowed sigma_J
                new_sigma_J_index += (np.random.randint(3)-1)
                new_sigma_J_index = new_sigma_J_index%(len(r_J.allowed_sigma_J)) # don't go out of bounds
                new_sigma_J = r_J.allowed_sigma_J[new_sigma_J_index]

	    elif np.random.random() < 0.48 :
                # take a step in array of allowed sigma_cs
                new_sigma_cs_H_index += (np.random.randint(3)-1)
                new_sigma_cs_H_index = new_sigma_cs_H_index%(len(r_cs_H.allowed_sigma_cs_H)) # don't go out of bounds
                new_sigma_cs_H = r_cs_H.allowed_sigma_cs_H[new_sigma_cs_H_index]
                new_sigma_cs_Ha_index += (np.random.randint(3)-1)
                new_sigma_cs_Ha_index = new_sigma_cs_Ha_index%(len(r_cs_Ha.allowed_sigma_cs_Ha)) # don't go out of bounds
                new_sigma_cs_Ha = r_cs_Ha.allowed_sigma_cs_Ha[new_sigma_cs_Ha_index]
                new_sigma_cs_N_index += (np.random.randint(3)-1)
                new_sigma_cs_N_index = new_sigma_cs_N_index%(len(r_cs_N.allowed_sigma_cs_N)) # don't go out of bounds
                new_sigma_cs_N = r_cs_N.allowed_sigma_cs_N[new_sigma_cs_N_index]
                new_sigma_cs_Ca_index += (np.random.randint(3)-1)
                new_sigma_cs_Ca_index = new_sigma_cs_Ca_index%(len(r_cs_Ca.allowed_sigma_cs_Ca)) # don't go out of bounds
                new_sigma_cs_Ca = r_cs_Ca.allowed_sigma_cs_Ca[new_sigma_cs_Ca_index]
		...
		# take a step in array of allowed sigma_PF
                ...
                new_gamma_index = new_gamma_index%(len(r_noe.allowed_gamma)) # don't go out of bounds
                new_gamma  = r_noe.allowed_gamma[new_gamma_index]
...
class PosteriorSamplingTrajectory(object):
    ...
    def process(self):
        ...
        self.results['allowed_sigma_noe']
        self.allowed_sigma_noe.tolist()
        self.results['allowed_sigma_J'] = r_J.allowed_sigma_J.tolist()
        self.results['allowed_sigma_cs_H'] = r_cs_H.allowed_sigma_cs_H.tolist()   #GYH
        self.results['allowed_sigma_cs_Ha'] = r_cs_Ha.allowed_sigma_cs_Ha.tolist()   #GYH
        self.results['allowed_sigma_cs_N'] = r_cs_N.allowed_sigma_cs_N.tolist()   #GYH
        self.results['allowed_sigma_cs_Ca'] = r_cs_Ca.allowed_sigma_cs_Ca.tolist()   #GYH
	self.results['allowed_sigma_PF'] = self.allowed_sigma_PF.tolist() #GYH
        self.results['allowed_gamma'] = r_noe.allowed_gamma.tolist()
	...
	self.results['sampled_alpha'] = self.sampled_alpha.tolist()	#GYH
        ...
        self.results['sigma_noe_mode'] = float(r_J.allowed_sigma_J
[ np.argmax(self.sampled_sigma_noe) ])
        self.results['sigma_J_mode']   = float(r_J.allowed_sigma_J[ np.argmax(self.sampled_sigma_J) ])
        self.results['sigma_cs_H_mode']   = float(r_cs_H.allowed_sigma_cs_H[ np.argmax(self.sampled_sigma_cs_H) ])      #GYH
        self.results['sigma_cs_Ha_mode']   = float(r_cs_Ha.allowed_sigma_cs_Ha[ np.argmax(self.sampled_sigma_cs_Ha) ])      #GYH
        self.results['sigma_cs_N_mode']   = float(r_cs_N.allowed_sigma_cs_N[ np.argmax(self.sampled_sigma_cs_N) ])      #GYH
        self.results['sigma_cs_Ca_mode']   = float(r_cs_Ca.allowed_sigma_cs_Ca[ np.argmax(self.sampled_sigma_cs_Ca) ])      #GYH
	self.results['sigma_PF_mode']	= float(self.allowed_sigma_PF[ np.argmax(self.sampled_sigma_PF) ])	#GYH
        self.results['gamma_mode']     = float(r_noe.allowed_gamma[ np.argmax(self.sampled_gamma) ])
