...
class Convergence(object):
    """
    """
    def __init__(self,trajfile = None, maxtau = 10000, nblock=5, nfold=10, nround=1000):
        if trajfile is None:
...
#                parameters[j].append(self.allowed_parameters[j][ind[j]])
    ...
    def plot_auto_curve(self, autocorrs, yFit, labels, fname = "autocorrelation_curve_with_exp_fitting.png"):
    ...
    def process(self, nblock=5, nfold=10, nrounds=100, savefile=True, plot=True, verbose = False, block = False):
    ...
    def plot_JSD_conv(self, JSD, JSDs):
        ...
        for k in range(len(JSD[0])):
            ...
            for j in range(len(JSD)):
                ...
                p = 1. * np.arange(len(all_JSDs))
                ind = np.where(JSDs_sorted==JSD[j][k])[0][0]
                plt.plot(JSDs_sorted[ind],p[ind],'o',ms=5,color='red')
                plt.plot(JSDs_sorted,p)
                plt.xlabel('JSD')
                plt.ylabel('$p$')
