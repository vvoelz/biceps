...
parser.add_argument("energy", help="energy file to use")
...
if (1):
    nclusters = 15036
#    nclusters = 71
    energies_filename = args.energy #'energy/micro/energy_-10.0.txt' #'reduced_free_energies_eps-3.000.dat'
#    energies_filename = 'energy/energy_10.0.txt' #'reduced_free_energies_eps-3.000.dat'
...
#    model_distances = loadtxt('mean_distances/mean_distances_state%d.dat'%i) # convert to A
    model_distances = loadtxt('micro_distances/dis_state%d.txt'%i)*10.0 # convert to A



#    s = Structure('Gens/Gens%d.pdb'%i, args.lam*energies[i], expdata_filename_noe=expdata_filename_noe, use_log_normal_distances=False, dloggamma=np.log(1.01), gamma_min=0.2, gamma_max=5.0, dalpha=0.1, alpha_min=0.0, alpha_max=0.1)

    s = Structure('Gens/micro/%d.pdb'%i, args.lam*energies[i], expdata_filename_noe=expdata_filename_noe, use_log_normal_distances=False, dloggamma=np.log(1.01), gamma_min=0.2, gamma_max=5.0, dalpha=0.1, alpha_min=0.0, alpha_max=0.1)
