...
class Structure(object):
    ...
    def __init__(self, PDB_filename, free_energy,
			expdata_filename_noe=None,
			expdata_filename_J=None,
			expdata_filename_cs_H=None,
			expdata_filename_cs_Ha=None,
			expdata_filename_cs_N=None,
			expdata_filename_cs_Ca=None,
			expdata_filename_PF=None,
...
#	self.dist_prior=dist_prior	# 05/2017
...
#       t=self.conf.atom_slice(Ind)
 ...
 #              model_chemicalshift = r.mean(axis=1)
    ...
    def add_protectionfactor_restraint(self, i, exp_protectionfactor, model_protectionfactor=None, verbose=False):
	"""Add a protectionfactor NMR_Protectionfactor() object to the list."""
	if verbose:
            print 'Adding protectionfactor_restraint for resid', i
	if model_protectionfactor == None:
	    ...
	    # model_protectionfactor = self.compute_PF_multi(self.Ncs[:,:,i], self.Nhs[:,:,i])
            model_protectionfactor = self.compute_PF_multi(self.Ncs[:,:,i], self.Nhs[:,:,i], debug=True)
...
#                print self.chemicalshift_Ca_restraints[i].weight
    ...
    def compute_sse_protectionfactor(self,debug=True):         #GYH
...
#	self.Ndof_protectionfactor = N
    ...
    def compute_neglog_reference_priors_PF(self, c=1.0):              #GYH
        """Uses the stored beta information (calculated across all structures) to calculate
        - log P_ref(PF[j]) for each protection factor restraint for (residue) j.

            # Each self.protectionfactor_restraints[j]  is a 6-dim array for each set of nuisance parameters --VAV
            #
            #     P( PF )  = min( np.exp(c*PF), 1.0 ) 
            # -ln P( PF )  = max( -c*(PF) , 0.0 )
            #
            #       where c is a slope parameter describing how severely negative PF values are penalized
        """

        # print 'self.betas', self.betas
#	print '(self.nprotectionfactor)', (self.nprotectionfactor)
        self.neglog_reference_priors_PF= np.zeros((self.nprotectionfactor,
						   len(self.allowed_beta_c), len(self.allowed_beta_h), len(self.allowed_beta_0),
						   len(self.allowed_xcs),    len(self.allowed_xhs),    len(self.allowed_bs)))
...
#	    print "self.protectionfactor_restraints[j]", self.protectionfactor_restraints[j]
            self.neglog_reference_priors_PF[j] = np.maximum(-1.0*self.protectionfactor_restraints[j].model_protectionfactor, 0.0)
...
#	print "self.sum_neglog_reference_priors_PF", self.sum_neglog_reference_priors_PF
#	print "self.sum_neglog_reference_priors_PF.shape", (self.sum_neglog_reference_priors_PF).shape
#	sys.exit()

    def compute_gaussian_neglog_reference_priors_PF(self):     #GYH
        """An alternative option to calculate log P_ref(PF[j]) for each protection factor
          restraint (residue j) based on a Gaussian distribution.  Here

            # Each self.protectionfactor_restraints[j]  is a 6-dim array for each set of nuisance parameters --VAV
            #
            #     P( PF )  = (2\pi\sigma^2)^(-0.5) np.exp( -(PF - mean_PF)^2 / 2*sigma^2 ) 
            # -ln P( PF )  = 0.5\ln(2\pi) + ln(\sigma) + (PF - mean_PF)^2 / 2*sigma^2
            #
        """
        ...
        for j in range(self.nprotectionfactor): # number of residues
            ...
            self.gaussian_neglog_reference_priors_PF[j] = 0.5*np.log(2.0*np.pi) + np.log(self.ref_sigma_PF[j]) \
                              + (self.protectionfactor_restraints[j].model_protectionfactor - self.ref_mean_PF[j])**2.0/(2*self.ref_sigma_PF[j]**2.0)
            self.sum_neglog_reference_priors_PF  += self.protectionfactor_restraints[j].weight * self.gaussian_neglog_reference_priors_PF[j]
