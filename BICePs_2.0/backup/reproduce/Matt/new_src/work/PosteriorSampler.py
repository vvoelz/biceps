...
class PosteriorSampler(object):
    ...
    def __init__(self, ensemble, freq_write_traj=1.,
...
#            for s in self.ensemble[rest_index]:
 ...
 #       self.parameter_indices = [197,162]
        self.sigma_indices = 197
        self.gamma_indices = 162
        # Store a list of parameters that correspond to the index inside a list
        # parameters e.g., [[1.2122652], [0.832136160], ...]
 #       self.parameters = [ ]
        self.sigma = 1.4201709698097866
        self.gamma = 1.0025188527717641
        #self.parameters=[1.4201709698097866,1.0025188527717641]
...
#        RAND = 1. - 1./(len(self.ensemble[0]) + 1.)
  ...
  #          new_para_index = self.new_para_index
            ...
            sigma_indices = self.sigma_indices
            ...
            if np.random.random() < 0.33:
                ...
                sigma_indices += (np.random.randint(3)-1)
#                index = np.random.randint(len(nuisance_para))
                # New index for specific parameter that belongs to a specific restraint
                sigma_indices = sigma_indices%(len(self.nuisance_para[0][0][0]))
                new_sigma = self.nuisance_para[0][0][0][sigma_indices]
...
#                print('parameter_indices:',parameter_indices)
                ...
                gamma_indices += (np.random.randint(3)-1)
#                index = np.random.randint(len(nuisance_para))
                # New index for specific parameter that belongs to a specific restraint
                gamma_indices = gamma_indices%(len(self.nuisance_para[0][0][1]))
                new_gamma = self.nuisance_para[0][0][1][gamma_indices]
            else:
            ...
            parameters = [[]]
            parameter_indices = [[]]
            parameters[0].append(new_sigma)
            parameters[0].append(new_gamma)
            parameter_indices[0].append(sigma_indices)
            parameter_indices[0].append(gamma_indices)
            # Compute new "energy"
...
#            print('accept',accept)
            ...
            #        self.traj.sampled_gamma[int(list(self.parameter_indices)[i][1])] += 1
            #    self.traj.sampled_sigmas[i][int(list(self.parameter_indices)[i][0])] += 1
            #self.traj.sampled_sigmas[new_rest_index][parameter_indices[new_rest_index][0]] += 1
            self.traj.sampled_sigmas[0][self.sigma_indices]+=1
            self.traj.sampled_gamma[self.gamma_indices]+=1
           ...
           # for i in range(len(self.ensemble[self.new_state])):
            ...
            if accept:
                ...
                self.sigma_indices = sigma_indices
            ...
            self.parameters = [[]]
            self.parameter_indices = [[]]
            self.parameters[0].append(self.sigma)
            self.parameters[0].append(self.gamma)
            self.parameter_indices[0].append(self.sigma_indices)
            self.parameter_indices[0].append(self.gamma_indices)
