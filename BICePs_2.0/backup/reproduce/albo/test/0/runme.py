...
#                R = Restraint_J('pdbs_guangfeng/%d.pdb'%i,ref='uniform', dlogsigma=np.log(1.01), sigma_min=0.05, sigma_max=20.0)  # good ref
                R = Restraint_J('pdbs_guangfeng/%d.pdb'%i,ref='uniform', dlogsigma=0.01, sigma_min=1.00, sigma_max=1.03) 
                R.prep_observable(lam=lam, free_energy=energies[i],
                        filename=File)

            elif File.endswith('noe'):
                R = Restraint_noe('pdbs_guangfeng/%d.pdb'%i,ref='exp',dlogsigma=0.01, sigma_min=1.05, sigma_max=1.08)   # good ref
                R.prep_observable(lam=lam, free_energy=energies[i],
                        filename=File, dloggamma=0.1,gamma_min=0.1,gamma_max=0.4)
