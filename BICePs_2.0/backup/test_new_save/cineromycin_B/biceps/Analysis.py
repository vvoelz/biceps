	...
	for n in range(nsnaps):
  		...
  		for k in range(self.K):
				...
				if debug:
          			...
          			states_kn[k,n] = state
				temp_parameters = []
				new_parameters=[[] for i in range(len(sigma_index))]
                                #print new_parameters

				temp_parameter_indices = np.concatenate(sigma_index)
				for inds in range(len(temp_parameter_indices)):
					temp_parameters.append(self.traj[k]['allowed_parameters'][inds][temp_parameter_indices[inds]])
