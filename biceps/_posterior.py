...
class PosteriorSampler(object):
    ...
    def __init__(self, ensemble, nreplicas=1, write_every=100, verbose=False):
        ...
        Args:
            ...
            write_every(int): the frequency (in steps) to write the MCMC trajectory
        ...
        self.logZs = [self.compute_logZ(ensemble) for ensemble in self.ensembles]
        self.model = self.get_model(self.ensembles[0])
        self.nstates = len(self.ensembles[0]) # ensemble is a list of Restraint objects
        self.nreplicas = nreplicas
        self.states = np.random.randint(low=0, high=self.nstates, size=self.nreplicas)
        self.E = 1.0e99
        self.indices = []
        for i,R in enumerate(self.ensembles[0][self.states[0]]):
...
#        self.nreplicas = nreplicas
#        self.traj_every = write_every # Frequency of storing trajectory samples
...
#        self.E = 1.0e99   # initial energy
        ...
        self.traj = PosteriorSamplingTrajectory(ensemble=self.ensemble, sampler=self, nreplicas=self.nreplicas)
...
#        self.model = self.get_model()
        #print(self.model)
        #mean = np.mean(np.transpose(self.model), axis=1)
        #self.mean_r = np.array([[np.mean(self.model[i][k], axis=0) for i in range(len(self.model))] for k in range(len(self.model[i]))])
        #self.mean_r = self.mean_r.transpose()
        #self.sem0 = np.array([np.std(mean[i]) for i in range(len(mean))])
        #print(f"self.sem0 = {self.sem0}")
        #self.sem0 /= np.sqrt(self.nreplicas)
        #print(f"self.sem0/sqrt(N) = {self.sem0}")
        #self.sem = self.sem0
        self.verbose = verbose
...
#        np.save('compiled_nuisance_parameters.npy',self.nuisance_para)
    ...
    def get_model(self):
        ...
        for s in range(self.nstates):
            for R in self.ensemble[s]:
                model[s].append(np.array([R.restraints[j]["model"] for j in range(R.n)]))
    ...
    def sample(self, int nsteps, int attempt_lambda_swap_every=0, int burn=0,
            bool swap_sigmas=False, int print_freq=1000,
            bool verbose=False, bool progress=True, bool multiprocess=True):
        ...
        self._sample(nsteps, burn, print_freq, verbose, progress)
        ...
        for l,lam in enumerate(self.lambda_values):
            accept = self.traj[l].sep_accept
            acceptance_info.append({"lambda": lam, "acceptance %": np.around(accept[1], decimals=1)[0]})
        self.acceptance_info = pd.DataFrame(acceptance_info)


    def save_trajectories(self, outdir, save_object=False):
        for l,lam in enumerate(self.lambda_values):
            self.traj[l].process_results(f"{outdir}/traj_lambda{lam}.npz", save_object=save_object)




## _sample:{{{
#    def _sample(ensemble, allowed, initial_state, initial_energy, , nsteps, burn=0, print_freq=1000, verbose=False, progress=True):
#        """Perform n number of steps (nsteps) of posterior sampling, where Monte
#        Carlo moves are accepted or rejected according to Metroplis criterion.
#        Energies are computed via :class:`neglogP`.
#
...
#        for i,R in enumerate(ensemble[state[0]]):
#            keys = R.__dict__.keys() # all attributes of the Child Restraint class
#            for j in [key for key in keys if "index" in key]: # get the parameter indices
#                indices.append(getattr(R, j))
#            for j in [key.split("_")[-1] for key in keys if "allowed_" in key]: #
#                rest_type.append(str(j)+"_"+str(R.__repr__).split("_")[-1].split()[0])
#                rest_index.append(i)
#        if verbose:
...
#        sep_accepted = np.zeros(len(indices)+1) # all nuisance paramters + state (n_para starts from 1 not 0)
#        step=0
#        start = time.time()
...
#            state, E = state.copy(), _E
#            indices = indices.copy() # e.g. [161, 142]
#            #values = self.values # e.g. [1.2122652, 0.832136160]
#            # All sample-space will share the same probability to be sampled
#
#            # NOTE: TODO: parameters = [dict(sample_sigma=True, sample_gamma=False)]
#
#            #_rest_ind = [0]
#            #RAND = 1. - 1./(len(_rest_ind) + 1.)   # + 1. is the term to include state-space
...
#                #for k in np.where(np.array(rest_index)==np.random.choice(_rest_ind))[0]:
...
#                state[np.random.randint(1)] = np.random.randint(low=0, high=nstates, size=1)
#                ind = [len(indices)]
...
#            E = 0
#            for s in state:
#                s = ensemble[int(s)] # Current Structure (list of restraints)
#                E += s[0].energy + self.logZ  # Grab the free energy of the state and normalize
#                for i,R in enumerate(s):
#                    E += R.compute_neglogP(parameters[i], parameter_indices[i], s[i].sse)
#
...
#                self.accept = True
#            else:
#                if np.random.random() < np.exp( _E - E ):
#                    self.accept = True
#
...
#                    self.traj.state_counts[int(state[i])] += 1
#                self.traj.state_trace.append(list(state.copy()))
#                # Store the counts of sampled sigma along the trajectory
#                for i in range(len(indices)):
#                    self.traj.sampled_parameters[i][indices[i]] += 1
#                # Store trajectory samples
#                temp=[[] for i in range(n_rest)]
#                for n,m in enumerate(rest_index):
#                    temp[m].append(indices[n])
#                # Store trajectory samples
#                if _step%self.traj_every == 0:
#                    self.traj.trajectory.append( [int(_step), float(_E),
#                        int(self.accept), list(state.copy()), list(temp.copy())])
#                        #int(self.accept), int(state), list(temp.copy())])
#                    self.traj.traces.append(self.values.copy())
#                    self.traj.sem_trace.append({str(rest_type[i]):sem for i,sem in enumerate(self.sem)})
#                    self.traj.sse_trace.append({str(rest_type[i]):sse for i,sse in enumerate(self.sse)})
#
#                if verbose:
#                    if _step%print_freq == 0:
#                        output = """%i\t\t%s\t%s\t\t%.3f\t\t%.2f\t%s"""%(_step, state,
#                                indices, _E/self.nreplicas, self.accepted/self.total*100., self.accept)
#                        print(output)
#            step += 1
#        restraints = list(dict.fromkeys([r.split("_")[-1] for r in rest_type]))
#        if (not verbose) and progress: pbar.close()
#
#        if verbose: print('\nAccepted %s %% \n'%(self.accepted/self.total*100.))
#        if verbose: print('\nAccepted [ ...Nuisance paramters..., state] %')
#        if verbose: print('Accepted %s %% \n'%(sep_accepted/self.total*100.))
...
class PosteriorSamplingTrajectory(object):
    def __init__(self, ensemble, sampler, nreplicas):
        ...
        self.state_trace = []
        self.sem_trace = [dict() for i in self.ensemble[0]]
        self.sse_trace = [dict() for i in self.ensemble[0]]
        s = self.ensemble[0]
    ...
    def process_results(self, filename='traj.npz'):
        ...
        #    try:
        #        self.results[str(key).split('.')[-1]] = val(object(key))
        #    except TypeError:
        #        print(f"Attempted: self.results['{str(key).split('.')[-1]}'] = {val}({key})")
        #        exit()
        ...
        df = pd.DataFrame(self.sem_trace)
        df.to_pickle(out)
        out = f"{outdir}/Sigma_SSE_trace_{filename.split('/')[-1].split('.npz')[0]}.pkl"
        df = pd.DataFrame(self.sse_trace)
        df.to_pickle(out)
        ...
        print(f"Time for processing: %.3f s" % (time.time() - stime));

        #save_object(filename, self.results)
