...
# Notes:{{{
'''
 We can add default arguments to the argparse
...
## Control:{{{
#parser = argparse.ArgumentParser(prog='BICePs',
...
# Testing with arbitrary ensemble:
ensemble = np.array([['State_0','State_1','State_2','State_3'],
        ['State_4','State_5','State_6','State_7'],
        ['State_8','State_9','State_10','State_11']])
# Using arbitrary 0 to 10 experimental data
expData = np.array([i for i in range(0, 10)])

def main():
    Sampler(ensemble).build_ensembles()
    # Lets give a random ensemble index and a random newstate
    new_ensemble_index = random.randrange(len(ensemble))
    print len(ensemble[new_ensemble_index])
    new_state = random.randrange(len(ensemble[new_ensemble_index])-1)
    Sampler(ensemble).neglogP(new_ensemble_index,new_state)

    # Now calling on the Restraint function:
    res = RestraintFn(ensemble)
    res.likelihood(Sampler, expData, new_ensemble_index, new_state)
